...
echo -e "${CYAN}=====================================================${NC}"
echo -e "${CYAN}  WireGuard Client setup script v.1.3 (C) torbik     ${NC}"
echo -e "${CYAN}=====================================================${NC}"
echo ""
...
# Определяет Docker bridge-интерфейсы и опубликованные порты контейнеров.
# Заполняет массив DOCKER_MANGLE_RULES с правилами вида "bridge proto port"
detect_docker_ports() {
    DOCKER_MANGLE_RULES=()
    ...
    # Собираем все Docker bridge-интерфейсы
    local BRIDGES=()
    while IFS= read -r net; do
        local br
        br=$(docker network inspect "$net" --format '{{if .Options}}{{index .Options "com.docker.network.bridge.name"}}{{end}}' 2>/dev/null)
        [ -n "$br" ] && BRIDGES+=("$br")
    ...
    # Собираем опубликованные порты: "proto port"
    local PORTS=()
    while IFS= read -r mapping; do
        # Формат: 0.0.0.0:HOST_PORT->CONT_PORT/PROTO или :::HOST_PORT->CONT_PORT/PROTO
        local proto port
        proto=$(echo "$mapping" | grep -oP '/\K(tcp|udp)')
        port=$(echo "$mapping" | grep -oP ':\K\d+(?=->)')
        if [ -n "$proto" ] && [ -n "$port" ]; then
            local entry="${proto} ${port}"
            # Дедупликация
            local dup=false
            for p in "${PORTS[@]}"; do [ "$p" = "$entry" ] && dup=true; done
            $dup || PORTS+=("$entry")
        fi
    done < <(docker ps --format '{{.Ports}}' 2>/dev/null | tr ',' '\n')

    # Строим правила: bridge + proto + port
    for br in "${BRIDGES[@]}"; do
        for pp in "${PORTS[@]}"; do
            DOCKER_MANGLE_RULES+=("${br} ${pp}")
        done
    done

    if [ ${#DOCKER_MANGLE_RULES[@]} -gt 0 ]; then
        echo -e "${CYAN}  Docker-порты для mangle:${NC}"
        for rule in "${DOCKER_MANGLE_RULES[@]}"; do
            echo "    -i $(echo $rule | awk '{print $1}') -p $(echo $rule | awk '{print $2}') --sport $(echo $rule | awk '{print $3}')"
        done
    fi
}
...
apply_routes() {
    ...
    # Локальные сервисы (SSH и т.д.) — OUTPUT chain
    ...
    # Docker-контейнеры — PREROUTING chain (ответный трафик идёт через bridge)
    detect_docker_ports
    for rule in "${DOCKER_MANGLE_RULES[@]}"; do
        local br proto port
        br=$(echo "$rule" | awk '{print $1}')
        proto=$(echo "$rule" | awk '{print $2}')
        port=$(echo "$rule" | awk '{print $3}')
        iptables -t mangle -A PREROUTING -i "$br" -p "$proto" --sport "$port" -j MARK --set-mark 0x1
    done
